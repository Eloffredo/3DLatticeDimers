...
def get_interaction_map(s1,s2):

    PATH_INTERACTING_MAP = f'tmp_contact_int_{s1}_{s2}.txt'
    assert os.path.isfile('./src/'+PATH_INTERACTING_MAP) or os.path.isfile('./'+PATH_INTERACTING_MAP), "Interaction map not found. Please generate it running contact_interaction_map.py"
    global Cint 
    Cint = np.loadtxt(f'./src/tmp_contact_int_{s1}_{s2}.txt',dtype='int')
    

#Cint = np.loadtxt(f'./tmp_contact_int_{s1}_{s2}.txt',dtype='int')
Cs = np.loadtxt("./src/contact_maps_10000.dat",dtype="int")
...
@jit(nopython=True)
def energybindtrue(f,sequence_1,sequence_2):
    en =0.0
    
    for i in range(Ncontface):
        en += imat[sequence_1[(Cint[(f*Ncontface)+i][1]-1)]][sequence_2[(Cint[(f*Ncontface)+i][2]-1)]]
...
@jit(nopython=True)
def pbind(sface,sequence_1,sequence_2):
    normalize=0.0
    for k in range(Ninterface):
        normalize += np.exp(-energybindtrue(k,sequence_1,sequence_2))
        
    pbind=np.exp(-energybindtrue(sface,sequence_1,sequence_2))/normalize
